...
const Character = ({ character, onGenerate }) => {
  return (
    <Card sx={{ backgroundColor: '#f5f5f5', margin: '20px', borderRadius: '15px' }}>
      <CardHeader title={character.name} titleTypographyProps={{ align: 'center' }} />
      <CardContent>
        ...
        <Typography variant="body1"><strong>Alignment:</strong> {character.alignment}</Typography>
        <Typography variant="body1"><strong>Description:</strong> {character.description}</Typography>
      </CardContent>
      <CardActions>
        <Button fullWidth variant="contained" color="primary" onClick={onGenerate}>
          New Character
        </Button>
      </CardActions>
    </Card>
